...
const generateCube = () => {
    ...
    array.forEach((side, sideIndex) => {
        const sideHtml = document.createElement('div')
        sideHtml.classList.add('side')

        for (let i = 1; i <= sideValBoxesWidth * sideValBoxesWidth; i++) {
            let block = false
            side.array.forEach(it => {
                if (it.slot === i){
                    addBox(sideHtml,  it.img, i, sideIndex)
                    block = true
                }
            })
            if (block){
            ...
            addBox(sideHtml, null, i, sideIndex)
        }

        cube.appendChild(sideHtml)
    })

    function addBox(sideHtml, img, index, sideIndex){
        const minBoxHtml = document.createElement('div')

        minBoxHtml.classList.add('side__box')
        minBoxHtml.setAttribute('data-id', 'side-' + sideIndex + '&id-' + index)

        if (index%2) {
            minBoxHtml.style.cssText = "--bg: #0f8282"
        }else{
            minBoxHtml.style.cssText = "--bg: #069A8E"
        }

        if (img){
            const minBoxImgHtml = document.createElement('img')
            minBoxImgHtml.setAttribute('src', img)
            minBoxImgHtml.setAttribute('alt', ' ')
            minBoxHtml.appendChild(minBoxImgHtml)
        }

        sideHtml.appendChild(minBoxHtml)
...
function scrollScale(){
    const valHeight =  3200
    const viewport123 = document.querySelector('.viewport')
    ...
    window.addEventListener('scroll', () => {
        ...
        let scaleVal = ((valHeight - scrollTop)/ valHeight) + 0.7
        console.log(scaleVal, 'ad')

        viewport123.style.cssText = "--scale: " + scaleVal * 1.1
    });
}
// scrollScale()
//
